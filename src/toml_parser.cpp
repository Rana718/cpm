...
ProjectConfig TomlParser::parse(const std::filesystem::path &toml_path) {
    ...
    while (std::getline(file, line)) {
        ...
        if (current_section == "project") {
            ...
            if (url.starts_with("github:")) url = url.substr(7);
            auto at = url.find('@');
...
void TomlParser::create_default(const std::filesystem::path &toml_path,
    ...
    file << "# Compiled libraries built from source inside .cpm/\n";
    file << "# Format: name = \"github:user/repo@version\"\n";
    file << "#\n";
    file << "# hiredis = \"github:redis/hiredis@v1.2.0\"\n\n";

    file << "[libs]\n";
    file << "# System libraries resolved via nix (headers + .so linked into .cpm/)\n";
    file << "# Format: name = \"nix-package-name\"\n";
    file << "# The nix package name is the nixpkgs attribute, e.g.:\n";
    file << "#\n";
    file << "# glew    = \"glew\"\n";
    file << "# opengl  = \"libGL\"\n";
    file << "# vulkan  = \"vulkan-loader\"\n";
    file << "# sdl2    = \"SDL2\"\n";
