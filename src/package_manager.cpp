...
void PackageManager::init(const std::string &project_name) {
  ...
  std::cout << "[cpm] Compiler: " << Config::get_compiler() << " "
            << Config::get_compiler_version() << "\n";
...
std::string PackageManager::resolve_latest_tag(const std::string &github_url,
                                               const std::string &name) {
  // Get all tags sorted by version (descending), filter out ^{} refs
  std::string tag_cmd =
      "git ls-remote --tags --sort=-v:refname " + github_url +
...
void PackageManager::resolve_system_dependency(const SystemDependency &dep) {
  ...
  if (fs::exists(built_cache) && !fs::is_empty(built_cache)) {
    ...
    if (fs::exists(built_cache / "include") &&
        !fs::is_empty(built_cache / "include"))
      has_content = true;
    if (fs::exists(built_cache / "lib") && !fs::is_empty(built_cache / "lib"))
      has_content = true;
...
bool PackageManager::build_from_source(
    const std::string &name, const std::filesystem::path &src_path,
  ...
  if (fs::exists(src_path / "cooking.sh")) {
    ...
    if (ret == 0) {
      ...
      if (!ninja_build_dir.empty()) {
        ...
        std::string build_cmd = compiler_env + "export PATH=\"" +
                                bin_dir.string() +
                                ":$PATH\" && "
                                "cd " +
                                src_path.string() + " && ninja -C " +
                                ninja_build_dir.string() + " -j$(nproc) 2>&1";
      ...
      std::string find_libs =
          "find " + (src_path / "build").string() +
          " -name '*.a' -not -path '*/_cooking/*' -exec cp {} " +
          (install_prefix / "lib").string() + "/ \\; 2>/dev/null";
      std::system(find_libs.c_str());
      ...
      for (const auto &cook_base :
           {src_path / "build", src_path / "build" / "release"}) {
        auto cooked = cook_base / "_cooking" / "installed";
        if (fs::exists(cooked / "include")) {
          std::string cp = "cp -r " + (cooked / "include").string() + "/* " +
                           (install_prefix / "include").string() +
                           "/ 2>/dev/null";
          std::system(cp.c_str());
        }
        if (fs::exists(cooked / "lib")) {
          std::string cp = "cp -r " + (cooked / "lib").string() + "/* " +
                           (install_prefix / "lib").string() + "/ 2>/dev/null";
          std::system(cp.c_str());
        }
      }
    ...
    if (fs::exists(src_inc) && !fs::is_empty(src_inc)) {
      ...
      for (const auto &cook_base :
        ...
        auto cooked = cook_base / "_cooking" / "installed";
        if (fs::exists(cooked / "include")) {
          std::string cp2 = "cp -r " + (cooked / "include").string() + "/* " +
                            (install_prefix / "include").string() +
                            "/ 2>/dev/null";
        ...
        if (fs::exists(cooked / "lib")) {
          fs::create_directories(install_prefix / "lib");
          std::string cp2 = "cp -r " + (cooked / "lib").string() + "/* " +
                            (install_prefix / "lib").string() + "/ 2>/dev/null";
          std::system(cp2.c_str());
        }
        ...
        if (fs::exists(ingredients)) {
          for (const auto &ing : fs::directory_iterator(ingredients)) {
            ...
            if (fs::exists(boost_hdrs)) {
              ...
              std::string cp3 = "cp -r " + boost_hdrs.string() + "/* " +
                                (install_prefix / "include" / "boost").string() + "/ 2>/dev/null";
              std::system(cp3.c_str());
            }
            // Other ingredients: headers at src/include/
            auto ing_include = ing_src / "include";
            if (fs::exists(ing_include)) {
              std::string cp3 = "cp -r " + ing_include.string() + "/* " +
                                (install_prefix / "include").string() + "/ 2>/dev/null";
              std::system(cp3.c_str());
            }
          }
  ...
  if (ret != 0 && fs::exists(src_path / "configure.py")) {
    ...
    if (cfg_ret == 0) {
      std::string build_cmd = "cd " + src_path.string() + " && " + build_tool +
                              ...
                              " && " +
                              build_tool +
                              " -C build/release install > /dev/null 2>&1";
      ret = std::system(build_cmd.c_str());
    }
  }
  ...
  if (ret != 0 && fs::exists(src_path / "CMakeLists.txt")) {
    ...
    std::string prefix_path = install_prefix.string();
    auto local_include = local_cpm_dir_ / "include";
    auto local_lib = local_cpm_dir_ / "lib";

    std::string cmake_cmd =
        "cd " + build_dir.string() + " && cmake " + src_path.string() +
        generator + " -DCMAKE_INSTALL_PREFIX=" + install_prefix.string() +
  ...
  // ─── Strategy 4: Makefile ───
  if (ret != 0 && (fs::exists(src_path / "Makefile") ||
                   fs::exists(src_path / "makefile"))) {
    std::cout << "[cpm]   → using make\n";

    std::string make_cmd =
        "cd " + src_path.string() + " && make -j$(nproc) > /dev/null 2>&1";
    ...
    if (ret == 0) {
      ...
      std::string install_cmd =
          "cd " + src_path.string() +
          " && make install PREFIX=" + install_prefix.string() +
          " > /dev/null 2>&1";
      ...
      if (install_ret != 0) {
        ...
        for (const auto &entry : fs::directory_iterator(src_path)) {
          if (entry.is_regular_file() && entry.path().extension() == ".a") {
            fs::copy(entry.path(),
                     install_prefix / "lib" / entry.path().filename(),
        ...
        if (!fs::exists(header_src))
          header_src = src_path / "src";

        if (fs::exists(header_src)) {
          for (const auto &entry :
               fs::recursive_directory_iterator(header_src)) {
  ...
  if (ret != 0 && fs::exists(src_path / "meson.build")) {
    ...
    std::string meson_cmd = "meson setup " + build_dir.string() + " " +
                            src_path.string() +
...
void PackageManager::install_built_library(
    const std::string &name, const std::filesystem::path &built_path) {
  namespace fs = std::filesystem;
...
void PackageManager::resolve_transitive_deps(
  ...
  if (fs::exists(cmake_file)) {
    ...
    while (std::getline(f, line)) {
      ...
      if (!pkg_name.empty() && pkg_name != "Threads" &&
          pkg_name != "PkgConfig") {
        // Skip cmake internal/utility modules (not real libraries)
        static const std::set<std::string> skip_modules = {
            "Threads",
            "PkgConfig",
            "PthreadSetName",
            "LinuxMembarrier",
  ...
  if (fs::exists(cmake_dir)) {
    for (const auto &entry : fs::directory_iterator(cmake_dir)) {
      ...
      if (fname.find("Find") == 0 &&
          fname.find(".cmake") != std::string::npos) {
        // FindYamlCpp.cmake → YamlCpp
        auto name =
            fname.substr(4, fname.size() - 10); // remove "Find" and ".cmake"
  ...
  std::cout << "[cpm]   → detected " << needed_packages.size()
  ...
  for (const auto &pkg : needed_packages) {
    ...
    std::transform(pkg_lower.begin(), pkg_lower.end(), pkg_lower.begin(),
                   ::tolower);
    ...
    if (fs::exists(install_prefix / "include")) {
      for (const auto &entry :
           fs::directory_iterator(install_prefix / "include")) {
    ...
    if (!found) {
      ...
      if (fs::exists(dep_inc)) {
        std::string cp_cmd = "cp -rn " + dep_inc.string() + "/* " +
                             (install_prefix / "include").string() +
                             "/ 2>/dev/null";
        std::system(cp_cmd.c_str());
      }
      if (fs::exists(dep_lib)) {
        std::string cp_cmd = "cp -rn " + dep_lib.string() + "/* " +
                             (install_prefix / "lib").string() +
                             "/ 2>/dev/null";
        std::system(cp_cmd.c_str());
      }
    }
...
std::string
PackageManager::search_github_repo(const std::string &package_name) {
  // Use GitHub API to search for the repository (top result by stars)
  std::string search_cmd =
      "curl -s 'https://api.github.com/search/repositories?q=" + package_name +
...
void PackageManager::install() {
  ...
  if (std::filesystem::exists(lib_dir)) {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(lib_dir)) {
      ...
      if (dot != std::string::npos)
        lib_name = lib_name.substr(0, dot);

      bool found = false;
      for (const auto &dep_name : wanted_libs) {
        // Match: dep name "hiredis" matches lib "hiredis" or "usockets" etc.
        if (lib_name.find(dep_name) != std::string::npos ||
            dep_name.find(lib_name) != std::string::npos) {
          found = true;
          break;
        }
      }
    ...
    for (const auto &lib_path : libs_to_remove) {
      std::cout << "[cpm] Removing " << lib_path.filename().string()
                << " (not in cpm.toml)\n";
  ...
  if (std::filesystem::exists(include_dir)) {
    ...
    for (const auto &path : includes_to_remove) {
      std::cout << "[cpm] Removing include: " << path.filename().string()
                << " (not in cpm.toml)\n";
...
std::filesystem::path
PackageManager::get_output_path(const ProjectConfig &config) const {
  std::string out_name = config.output.empty() ? config.name : config.output;
  return project_root_ / out_name;
}

std::string
PackageManager::build_compile_command(const ProjectConfig &config) const {
...
int PackageManager::build(bool static_build) {
  ...
  if (config.entry.empty()) {
    throw std::runtime_error(
        "No 'entry' specified in [project] section of cpm.toml");
  }
...
int PackageManager::run() {
  ...
  if (!config.git_dependencies.empty() || !config.system_dependencies.empty()) {
    ...
    if (!std::filesystem::exists(packages_dir) &&
        !config.git_dependencies.empty()) {
      needs_install = true;
    } else if (!std::filesystem::exists(include_dir) ||
               std::filesystem::is_empty(include_dir)) {
      needs_install = true;
    } else {
      for (const auto &dep : config.git_dependencies) {
        auto pkg_path = packages_dir / dep.name;
        if (!std::filesystem::exists(pkg_path) &&
            !std::filesystem::is_symlink(pkg_path)) {
          needs_install = true;
          break;
        }
      }
...
void PackageManager::install_package(const std::string &package_spec) {
  ...
  if (at_pos != std::string::npos) {
    ...
    dep.name =
        (slash_pos != std::string::npos) ? repo.substr(slash_pos + 1) : repo;
  } else {
    dep.github_url = "https://github.com/" + spec;
    dep.version = "*";
    auto slash_pos = spec.find('/');
    dep.name =
        (slash_pos != std::string::npos) ? spec.substr(slash_pos + 1) : spec;
  }
...
void PackageManager::remove_package(const std::string &package_name) {
  ...
  if (!std::filesystem::exists(package_path) &&
      !std::filesystem::is_symlink(package_path)) {
