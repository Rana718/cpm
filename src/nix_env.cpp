...
std::string NixEnv::generate_shell_nix(const std::string &compiler, const std::string & /*cpp_standard*/,
    ...
    if (!user_nix_config.empty() && fs::exists(user_nix_config)) {
        ...
        std::set<std::string> user_packages;
        static const std::regex re_pkg_block(R"(with\s+pkgs\s*;\s*\[([^\]]*)\])", std::regex::multiline);
        static const std::regex re_token(R"([A-Za-z][A-Za-z0-9_.+\-]*)");
        std::smatch block;
        ...
        expr << "# Generated by CPM — user nix_config merged with CPM dependencies\n"
             << "{ pkgs ? import <nixpkgs> {} }:\n"
             << "let\n"
             << "  userShell = import " << fs::absolute(user_nix_config).string() << " { inherit pkgs; };\n"
             << "  cpmExtra  = with pkgs; [";
        for (const auto &pkg : additional) expr << ' ' << pkg;
        expr << " ];\n"
    ...
    // No user config — generate a plain CPM-managed shell.
    std::ostringstream expr;
    expr << "{ pkgs ? import <nixpkgs> {} }:\npkgs.mkShell {\n  packages = with pkgs; [\n";
    for (const auto &pkg : cpm_packages) expr << "    " << pkg << '\n';
    expr << "  ];\n}\n";
    return expr.str();
}
