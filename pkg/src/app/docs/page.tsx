   ...
   return (
      ...
      >
         {copied ? (
            ...
            <AiOutlineCopy className="h-3.5 w-3.5" />
         )}
      </button>
   );
}
...
function CodeBlock({ code, lang = "bash" }: { code: string; lang?: string }) {
   ...
   return (
      <div className="relative group my-3 rounded-lg overflow-hidden border border-border">
         <div className="flex items-center justify-between px-3 py-1.5 bg-zinc-800 dark:bg-zinc-900">
         ...
         {mounted ? (
            ...
            <pre className="bg-muted px-4 py-4 text-sm font-mono overflow-x-auto">
               <code>{code}</code>
            </pre>
         )}
      </div>
...
function Callout({
   ...
   return (
      <div
         className={`border rounded-lg px-4 py-3 my-3 text-sm ${styles[type]}`}
      >
   ...
   return (
      <div className="border border-border rounded-lg overflow-hidden">
         ...
         >
            {q}
            {open ? (
               <AiOutlineDown className="h-4 w-4 flex-shrink-0" />
            ) : (
               <AiOutlineRight className="h-4 w-4 flex-shrink-0" />
            )}
         </button>
         {open && (
...
const COMMANDS = [
   ...
   {
      cmd: "cpm build -s",
      desc: "Production build — optimized + portable bundle in dist/",
   },
   { cmd: "cpm run", desc: "Install + build + run in one command" },
   {
      cmd: "cpm run file.cpp",
      desc: "Compile and run a single file (no project needed)",
   },
   { cmd: "cpm start", desc: "Run the already-built binary" },
   {
      cmd: "cpm add <pkg>",
      desc: "Add a package, e.g. cpm add github:nlohmann/json",
   },
...
export default function DocsPage() {
   ...
   useEffect(() => {
      ...
      // We keep a map of which sections are currently intersecting
      const visibilityMap: Record<string, boolean> = {};

      const updateActive = () => {
         // Pick the first (topmost) section that is currently visible
      ...
      const observer = new IntersectionObserver(
         ...
         {
            // Fire when top of section is within the upper 30% of the viewport
   ...
   const scrollTo = (id: string) => {
      // Pause observer-driven updates while smooth-scrolling to avoid flicker
   ...
   return (
      <div className="flex flex-1 overflow-x-hidden">
         ...
         <main className="flex-1 min-w-0 px-8 lg:pl-[calc(15rem+2rem)] lg:pr-12 py-10 w-full">
            {/* Hero */}
            <div className="mb-10">
            ...
            <Section id="install">
               ...
               <p className="text-sm text-muted-foreground">
                  This installs system build tools (cmake, ninja, g++), Nix for
                  isolated builds, and the cpm binary to{" "}
               ...
               <CodeBlock
                  code={`git clone https://github.com/Rana718/cpm.git
cd cpm
...
sudo cp cpm /usr/local/bin/`}
               ...
               <Callout type="note">
                  cpm requires Linux x86_64, git, curl, and Nix
                  (auto-installed).
               </Callout>
            </Section>
...
cpm run`}
               ...
               <CodeBlock
                  code={`# No project needed — uses system g++
cpm run hello.cpp
cpm run test.c`}
               />
            </Section>
            ...
            <Section id="config">
               ...
               <p className="text-sm text-muted-foreground mb-3">
                  The project manifest — equivalent to{" "}
                  <code className="bg-muted px-1 rounded font-mono text-xs">
                     package.json
                  </code>{" "}
                  or{" "}
            ...
            <Section id="commands">
               ...
               <div className="flex flex-col gap-2">
                  {COMMANDS.map(({ cmd, desc }) => (
                     <div
                        key={cmd}
                        className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 rounded-lg border border-border px-4 py-3 hover:bg-muted/30 transition-colors"
                     >
                        <code className="font-mono text-sm text-primary whitespace-nowrap flex-shrink-0">
                           {cmd}
                        </code>
...
└── defines.txt     ← compiler flags`}
            ...
            <Section id="build">
               ...
               <CodeBlock
                  code={`cpm run          # install + build + run
...
└── run.sh      ← portable launcher`}
               ...
               <p className="text-sm text-muted-foreground">
                  Put additional source files in a{" "}
                  <code className="bg-muted px-1 rounded font-mono text-xs">
                     src/
                  </code>{" "}
                  directory. cpm automatically picks them up.
               </p>
               <CodeBlock
                  lang="text"
                  code={`myapp/
            ...
            <Section id="faq">
               ...
               <div className="flex flex-col gap-2">
                  <AccordionItem q="Does cpm work on macOS or Windows?">
                     Currently Linux x86_64 only. macOS and Windows support is
                     planned.
                  </AccordionItem>
                  <AccordionItem q="Will cpm touch my system libraries?">
                     No. Everything is stored in{" "}
                     <code className="bg-muted px-1 rounded font-mono text-xs">
                        .cpm/
                     </code>{" "}
                     (local) and{" "}
                  ...
                  <AccordionItem q="Why does cpm use Nix?">
                     Nix provides pre-built binaries from{" "}
                     <code className="bg-muted px-1 rounded font-mono text-xs">
                        cache.nixos.org
                     </code>{" "}
                     — most compiled deps don't need local compilation. It also
                     guarantees the correct compiler version and reproducible
                     builds.
                  </AccordionItem>
                  ...
                  <AccordionItem q="How do I find the right package name for cpm.toml?">
                     Search on this registry site, then copy the snippet
                     directly from the Install tab on the package page.
                  </AccordionItem>
