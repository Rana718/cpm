...
class NixEnv {
  public:
    ...
    std::string generate_shell_nix(const std::string &compiler, const std::string &cpp_standard, const std::vector<std::string> &extra_deps = {});

    // Build a library inside nix-shell and install to prefix
    bool build_in_shell(const std::filesystem::path &src_path, const std::filesystem::path &install_prefix, const std::string &shell_nix_content);
    ...
    void link_nix_packages(const std::vector<std::string> &packages, const std::filesystem::path &include_dir, const std::filesystem::path &lib_dir);

    // Run a command inside nix-shell (used by cpm build for linking)
    int run_in_shell(const std::string &cmd, const std::filesystem::path &shell_nix_path);
